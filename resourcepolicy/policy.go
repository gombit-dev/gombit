...
// no database default, and not auto-generated.
...
func Resolve(facts FieldFacts, gombitTag string) (Resolved, error) {
	...
	if facts.autoGenerated() {
		...
		if tag.present {
			...
			r.InResponse = !facts.SoftDelete // PK and timestamps read; soft-delete hidden
		}
		return r, nil
	}

	// A regular content column or a manual (non-auto) primary key: it can be read,
	// written from the request, or set server-side.
	if tag.present {
		r.InResponse = tag.read
