...
// key is implicitly so), no database default, and not auto-generated. This is
// INDEPENDENT of Creatable — Creatable only says whether GORM writes the column
// on INSERT, not whether anything supplies its value. A non-creatable required
// column with no default/auto-generation is therefore unsatisfiable, which is a
// distinct (worse) failure handled by Resolve, not a reason to skip the check.
...
func Resolve(facts FieldFacts, gombitTag string) (Resolved, error) {
	...
	// A required column must get a value at create time. What's wrong when it
	// can't depends on whether GORM will even write it:
	if facts.requiresCreateValue() {
		if !facts.Creatable {
			// GORM omits it from INSERT and nothing generates it → every create
			// hits the NOT NULL constraint. Reject the model outright.
		...
		if r.CreateSource == CreateSourceNone {
			// Creatable but the policy gives it no source → silent zero-fill (#352).
