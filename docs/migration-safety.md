...
classifies them structurally (the schema before and after each one) and fails
on a destructive or unsafe change the migration does not acknowledge with a
`-- gombit:allow` line. To see
