...
| `regex=` | `string`, `text` | unanchored request `pattern` and the form's `new RegExp` check, including `text`. The pattern must compile in Go RE2. The gate rejects inline flags, POSIX classes, RE2-only groups, `\Q` `\E` `\A` `\z` `\Z`, `\p`, `\P`, `\s`, `\S`, and the brace hex escape `\x{HHHH}` (two-digit `\xNN` is kept). `.` is still allowed: the request matches every character except `\n`, and the form also excludes `\r`, U+2028, and U+2029. The form does not set an HTML `pattern` attribute, because that attribute anchors the match. Not a SQL check |
