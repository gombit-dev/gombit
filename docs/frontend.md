...
the same with `raw === "" ? 0 : Number(raw)`. A `nullable` `uint` (such as a
nullable foreign key) is a pointer (`*uint`), so both presets submit `null` for
it instead. A nullable `int`, `int64`, or `float` is not a pointer and still
submits `0`.
