...
```text
keep   internal/task/task.go
create internal/task/.gombit-resource
...
write  internal/task/dto.gen.go
write  internal/task/handler.gen.go
seed   internal/task/hooks.go
```

`make resource` is **model-first** (see
[ADR-016](adr/016-model-first-resource-generation.md)): the model is the
human-owned source of truth, and everything else is derived from it.

- Your hand-written `task.go` is **kept** — the model is yours, scaffolded once
  and never overwritten (that is why there is no clobber prompt; `--force`
  re-scaffolds it from the CLI spec if you want).
- `dto.gen.go` and `handler.gen.go` are **generator-owned** (`DO NOT EDIT`):
  the request/response DTOs, mappers, and the Huma list/get/create handler,
  regenerated from the model by `gombit generate`, which `make resource` just
  ran for you.
...
- **Regeneration is the synchronization mechanism.** Change the model and re-run
  `gombit generate`; the `*.gen.go` are re-derived, and `gombit generate --check`
  fails if a committed copy is stale. `--dry-run` previews a run without writing.

`--service` and `--repo` add pass-through service/repository layers. The
default is a thin generated handler straight over GORM — add the layers when you
have a reason, not by default.

The generated `handler.gen.go` shows what was mounted:
...
the API contract** — but you don't hand-write it. It is derived from the model
into `handler.gen.go` / `dto.gen.go`, and OpenAPI is emitted from that code,
never hand-written.

From the generated `dto.gen.go` / `handler.gen.go`:
...
`minLength` (a NOT NULL string) and `maxLength` (the column size) are derived
from the model, land in the OpenAPI schema, **and** are enforced at
